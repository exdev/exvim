...
augroup perforce
  ...
  map <silent> <Leader>e :call <SID>P4OpenFileForEdit(0)<CR>
  map <silent> <Leader>E :call <SID>P4OpenFileForEdit(1)<CR>
  map <silent> <Leader>r :call <SID>P4RevertFile()<CR>
  map <silent> <Leader>i :echo <SID>P4GetFileStatus()<CR>
  map <silent> <Leader>s :echo <SID>P4SubmitChangelist(0)<CR> " For backward compatibility
  ...
  map <silent> <Leader>S :call <SID>P4SubmitChangelist(1)<CR>
  ...
  menu <silent> &Perforce.&Edit :call <SID>P4OpenFileForEdit(0)<CR>
  ...
  menu <silent> &Perforce.Submit\ changelist :call <SID>P4SubmitChangelist(1)<CR>
...
" Open a file for editing, with more checking than just wrapping the command
"----------------------------------------------------------------------------
function s:P4OpenFileForEdit(listChangeList)
    call s:P4GetFileStatus()
...
"----------------------------------------------------------------------------
function s:P4SubmitChangelist(blistChangeList)
    call s:P4GetFileStatus()

    if (a:blistChangeList == 1 )
        let listnum = ""
        let listnum = s:P4GetChangelist( "Current changelists:\n" . s:P4GetChangelists(0) . "\nEnter changelist number: ", b:changelist )
        if listnum == ""
        ...
        let result = s:P4ShellCommand("submit")
        if v:errmsg != ""
            echoerr "Unable to submit " . v:errmsg
            return
        else
            e!
            call s:P4RestoreLastPosition()   
